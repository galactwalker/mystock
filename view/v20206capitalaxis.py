...
class CapitalAxisItem:
    def __init__(self, data_capitalaxis_list):

        self.setData(data_capitalaxis_list)

        self.axis = self.createAxis()

    def setData(self, data_capitalaxis_list):
        self.data_tradedate = data_capitalaxis_list[0]        # 与kline_tradedate相同
        self.data_axis_length = data_capitalaxis_list[1]      # 与kline length相同
    ...
    def createAxis(self):
        ...
        capital_axis = AxisItem(orientation='bottom')  # 创建一个刻度项
        capital_axis.setTicks([axis_4, axis_3, axis_2, axis_1])  # 设置X轴刻度值，似乎最多设置4层，多了显示不了。一旦使用setTicks后，其他方法如tickstring/tickvalue/setstyle/setTickSpacing都不好用了，因为setTicks()会重写这些方法
